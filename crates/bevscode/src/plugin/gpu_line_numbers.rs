...
type GpuLineNumbersQuery<'w, 's> = Query<
    'w,
    's,
    (
        Entity,
...
pub(crate) fn update_gpu_line_numbers(
    ...
    editor_query: GpuLineNumbersQuery,
    ...
    ) in editor_query.iter()
    {
        ...
        if !fold_changed {
            ...
            if let Some((entity, batch)) = existing_batch_for_editor {
                ...
                if !scroll_changed
                    && !viewport_changed
                    && batch.built_at_version == buffer.content_version
                {
        ...
        if instances.is_empty() {
            if let Some((entity, _)) = batch_query.iter().find(|(_, b)| b.editor == editor_entity) {
                commands.entity(entity).insert(Visibility::Hidden);
            }
            continue;
        }
