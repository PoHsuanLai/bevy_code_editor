//! Installs the shared tokio runtime and routes LSP transport traffic
//! onto the Bevy message bus.
//!
//! Every variant of [`LspResponse`] gets its own [`Message`] type so hosts
//! subscribe to just the shape they care about. The drain pipeline is
//! split into two stages so the writer fan-out can stay under Bevy's
//! 16-parameter system cap and so that the reader systems can run in
//! parallel against the next frame's drain.
...
/// the outbound response messages, drives the drain pipeline each frame,
...
impl Plugin for LspPlugin {
    fn build(&self, app: &mut App) {
        ...
        // Register every Bevy message generated by `messages.rs`.
...
/// systems against the next drain. With ~50 message types we partition
/// the writers across five `flush_*` systems.
