...
pub fn render_layout(
    ...
    // Block-level backgrounds + borders. Drawn before per-row line_bg so a
    // line_bg painted on a row inside a decorated block stacks on top
    // (e.g. find-match highlight inside a code block).
...
/// content area (left = `content_start_x + indent`, right = viewport's
/// right edge minus content margin), so panels stretch across the
...
fn push_block_decorations(
    ...
    for block in layout.block_rects.iter() {
        // Find the first and last shaped row inside the block. Visible-window
        // culling can drop one or both — skip the block if neither is present.
        let first = layout
            .lines
            .iter()
            .find(|l| l.display_row == block.display_row_start);
        let last = layout
            .lines
            .iter()
            .rev()
            .find(|l| l.display_row == block.display_row_end);
        let (first, last) = match (first, last) {
            (Some(f), Some(l)) => (f, l),
            _ => continue,
        };

        let top_y = first.y_top - block.padding_top;
        let last_h = last.line_height.unwrap_or(default_line_height);
        let bottom_y = last.y_top + last_h + block.padding_bottom;
        let height = (bottom_y - top_y).max(1.0);

        let left_x = content_start_x + block.indent;
        let right_x = (viewport.width as f32 - content_start_x).max(left_x + 1.0);
        let width = right_x - left_x;
        let center_y = (top_y + bottom_y) * 0.5;
        let world_x = world_left + left_x + width * 0.5;
        let world_y = world_top - center_y;

        if let Some(bg) = block.decoration.background {
        ...
        if let Some(border) = block.decoration.border {
            let bw = border.width.max(0.0);
            if bw > 0.0 {
                let bc = linear_rgba(border.color);
                // Top edge — full width at the top.
                out.push(GlyphInstance {
                    position: Vec2::new(world_x, world_top - top_y - bw * 0.5),
                    uv_min: solid_uv.uv_min,
                    uv_max: solid_uv.uv_max,
                ...
                // Bottom edge.
                out.push(GlyphInstance {
                    position: Vec2::new(world_x, world_top - bottom_y + bw * 0.5),
                    uv_min: solid_uv.uv_min,
                    uv_max: solid_uv.uv_max,
                    size: Vec2::new(width, bw),
                    color: bc,
                ...
                out.push(GlyphInstance {
                    position: Vec2::new(world_left + left_x + bw * 0.5, world_y),
                    uv_min: solid_uv.uv_min,
                    uv_max: solid_uv.uv_max,
                ...
                // Right edge.
                out.push(GlyphInstance {
                    position: Vec2::new(world_left + right_x - bw * 0.5, world_y),
                    uv_min: solid_uv.uv_min,
                    uv_max: solid_uv.uv_max,
                    size: Vec2::new(bw, height),
                    color: bc,
