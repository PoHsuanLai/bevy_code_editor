...
pub struct CodeEditorState {
    ...
    #[cfg(feature = "tree-sitter")]
    pub cached_tree: Option<tree_sitter::Tree>,

    /// Pending edits to apply to the tree before re-parsing (for incremental highlighting)
    #[cfg(feature = "tree-sitter")]
    pub pending_edits: Vec<tree_sitter::InputEdit>,

    /// Cached tree-sitter parser (reused for performance)
    #[cfg(feature = "tree-sitter")]
    pub cached_parser: Option<tree_sitter::Parser>,

    /// Cached tree-sitter language (for incremental parsing)
    #[cfg(feature = "tree-sitter")]
    pub cached_language: Option<tree_sitter::Language>,
    ...
    /// Whether syntax highlighting is enabled and producing tokens
    /// When false, rendering reads directly from rope for O(visible_lines) performance
...
impl CodeEditorState {
    ...
    /// Update syntax highlighting tokens using incremental parsing
    #[cfg(feature = "tree-sitter")]
    pub fn update_highlighting(&mut self) {
        // Skip if content hasn't changed since last highlight
        if self.content_version == self.last_highlighted_version && !self.tokens.is_empty() {
            return;
        }
        ...
        if let Some(ref mut tree) = self.cached_tree {
            ...
            if let Some(ref mut parser) = self.cached_parser {
                if let Some(new_tree) = parser.parse(text_bytes, Some(tree)) {
                    *tree = new_tree;
                }
            }
            ...
            if let Some(ref mut parser) = self.cached_parser {
                self.cached_tree = parser.parse(text_bytes, None);
            }
        }

        // Now use tree-sitter-highlight for the actual highlighting
        // (This still does its own parse, but we maintain the cached tree for future incremental use)
        self.update_highlighting_from_config(&text);

        self.last_highlighted_version = self.content_version;
        self.dirty_lines = None;
    }

    /// Internal: Run highlighting using the HighlightConfiguration
    #[cfg(feature = "tree-sitter")]
    fn update_highlighting_from_config(&mut self, text: &str) {
        use tree_sitter_highlight::{Highlighter as TSHighlighter, HighlightEvent};

        // These are the actual capture names from tree-sitter grammars
        ...
        if let Some(config) = &self.highlight_config {
            ...
            let highlights = match ts_highlighter.highlight(config, text.as_bytes(), None, |_| None) {
                ...
                Err(e) => {
                    ...
                    // Fall back to direct rope reading
                    self.tokens.clear();
                    self.has_syntax_highlighting = false;
                    return;
                }
            };
            ...
            if tokens.is_empty() {
                // No tokens produced - fall back to direct rope reading
                self.tokens.clear();
                self.has_syntax_highlighting = false;
            } else {
                self.tokens = tokens;
            ...
            // No highlight config - rendering will read directly from rope
            self.tokens.clear();
            self.has_syntax_highlighting = false;
        }
    }

    /// Set the highlight configuration and language for syntax highlighting.
    /// This is the preferred way to configure highlighting, as it sets up both
    /// the configuration and the language for incremental parsing.
    #[cfg(feature = "tree-sitter")]
    pub fn set_highlight_config(&mut self, config: HighlightConfiguration, language: tree_sitter::Language) {
    ...
    /// Record an edit for incremental parsing (call this when text changes)
    #[cfg(feature = "tree-sitter")]
    pub fn record_edit(&mut self, start_byte: usize, old_end_byte: usize, new_end_byte: usize) {
        // Convert byte positions to tree-sitter Points
        let start_line = self.rope.byte_to_line(start_byte.min(self.rope.len_bytes()));
        let start_col = start_byte.saturating_sub(self.rope.line_to_byte(start_line));

        let old_end_line = self.rope.byte_to_line(old_end_byte.min(self.rope.len_bytes()));
        let old_end_col = old_end_byte.saturating_sub(self.rope.line_to_byte(old_end_line));

        let new_end_line = self.rope.byte_to_line(new_end_byte.min(self.rope.len_bytes()));
        let new_end_col = new_end_byte.saturating_sub(self.rope.line_to_byte(new_end_line));
    ...
    /// OPTIMIZATION: When tree-sitter is disabled, we don't create tokens.
    /// Instead, the rendering loop reads directly from rope for visible lines only.
    #[cfg(not(feature = "tree-sitter"))]
    pub fn update_highlighting(&mut self) {
        // Don't create tokens - rendering will read directly from rope
        // This avoids O(n) rope.to_string() call
        self.tokens.clear();
        self.has_syntax_highlighting = false;
        self.dirty_lines = None;
    }
    ...
    pub fn insert_char_with_history(&mut self, c: char, record_history: bool) {
        ...
        // Record edit for incremental parsing BEFORE making the change
        #[cfg(feature = "tree-sitter")]
        {
            let start_byte = self.rope.char_to_byte(cursor_pos);
            let char_len = c.len_utf8();
            self.record_edit(start_byte, start_byte, start_byte + char_len);
        }
    ...
    pub fn delete_backward_with_history(&mut self, record_history: bool) {
        if self.cursor_pos > 0 && self.cursor_pos <= self.rope.len_chars() {
            ...
            // Record edit for incremental parsing BEFORE making the change
            #[cfg(feature = "tree-sitter")]
            self.record_edit(char_idx, byte_idx_end, char_idx);
    ...
    pub fn insert_text_at(&mut self, pos: usize, text: &str) {
        ...
        #[cfg(feature = "tree-sitter")]
        {
            ...
            self.record_edit(start_byte, start_byte, start_byte + text_len);
        }
    ...
    pub fn remove_range(&mut self, start: usize, end: usize) {
        ...
        if start < end {
            ...
            // Record edit for incremental parsing
            #[cfg(feature = "tree-sitter")]
            self.record_edit(start_byte, end_byte, start_byte);
