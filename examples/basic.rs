...
fn setup_editor(mut editor_query: Query<(&mut CodeEditorState, &mut CursorState, &mut TextViewState), With<CodeEditor>>) {
...
"#;
    ...
    state.set_text(&mut cursor, &mut tv, initial_text);
}
